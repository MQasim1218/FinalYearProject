...
const LineChart = ({ isDashboard = true }) => {
    ...
    const { data: donorDons, isError: isDonorDonsErr, isLoading: isDonorDonsLoading, isSuccess: isDonorDonsSuccess, error: donorDonsErr } = useSingleAdminDonationsQuery(id)

    console.log(donorDons)
    ...
    donorDons?.forEach((don) => {
        const { category, amount, createdAt } = don;
        ...
        if (donationYear === currentYear) { // If the donation was made in a different year, skip it
            ...
            // If the category doesn't exist in lineChartData, create an empty array for it
            if (!lineChartData[category]) {
                lineChartData[category] = [];
                // Populate the array with objects for each month, starting with 0 values
                months.forEach((month) => {
                    lineChartData[category].push({ x: month, y: 0 });
                });
            }

            // Update the corresponding month's value in the lineChartData array
            lineChartData[category][month].y += amount;
        }
    });
