...
import {
    thunkFetch_asyncDelete,
    thunkFetch_asyncGet,
    thunkFetch_asyncPost,
    thunkFetch_asyncPatch,
    thunkFetch_asyncPut
} from '../../thunkFetch'
...
const initialState = {
    ...
    // Get Backend API Urls from the User Frontend as Key_Value Pairs
    // Key +> Identify the reducer that requires the route
    // Value +> The Url Endoint string
    apiUrls: {},
    adminDetails: {
        name: '',
        age: -1,
...
// export const fetchAdminDetails = createAsyncThunk('admin_donations/fetchAdminDetails', async (admin_id) => {
...
export const fetchAdminDetails = thunkFetch_asyncGet('adminIndv/fetchAdminDetails', initialState.apiUrls['admin_details'])
export const fetchAdminDonations = thunkFetch_asyncGet('adminIndv/fetchAdminDonations')
...
const adminSlice = createSlice({
    ...
    reducers: {
        ...
        makeDonation(state, action) {
            state.adminDonations.push(action.payload)
        },

        setApiUrls(state, action) {
            // Here what I am trying to do is to add another url (Key-Value) to the already existing url Objects
            state.apiUrls = { ...state.apiUrls, ...action.payload }
        }
    },
    extraReducers: b => {
        /** 
        ...
        b.addCase(fetchAdminDonations.fulfilled, (state, action) => {
            state.loading = false
            state.adminDonations = action.payload
            state.error = ''
        })
        b.addCase(fetchAdminDonations.rejected, (state, action) => {
            state.loading = false
            state.adminDonations = []
            state.error = action.error.message
        })
    }
})
